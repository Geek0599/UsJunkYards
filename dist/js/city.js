(() => {
    ...
    function rowVerticalAnim() {
        setTimeout((() => {
            ...
            if (itemsWrappers.length > 0) itemsWrappers.forEach((wrapper => {
                const itemsBody = wrapper.querySelector("[data-animation-items]");
                const minItems = itemsBody?.dataset.minItems;
                let offset = parseFloat(window.getComputedStyle(itemsBody).getPropertyValue("row-gap"));
                if (!itemsBody || minItems && itemsBody.children.length < minItems) {
                    wrapper.classList.add("_not-anim");
                    return;
                }
                const duration = +itemsBody.dataset.duration || 3e3;
                ...
                let itemsForMoving = allElements.slice(0, rowBy);
                function getRowBy(itemsBody) {
                    let rowBy = itemsBody.dataset.rowByMinWidth;
                    if (rowBy) {
                        let adaptiveObject = parseDataRowBy(rowBy);
                        let screenWidth = window.innerWidth;
                        let bestMatch = adaptiveObject.filter((obj => screenWidth >= obj.minWidth)).sort(((a, b) => b.minWidth - a.minWidth))[0];
                        return bestMatch ? bestMatch.rowBy : 1;
                    } else {
                        rowBy = 1;
                        return rowBy;
                    }
                }
                function parseDataRowBy(attributeValue) {
                    return attributeValue.split(", ").map((item => {
                        const [rowBy, minWidth] = item.split(" : ").map((val => val.trim()));
                        return {
                            rowBy: Number(rowBy),
                ...
                function getElementsHeight(allElements, rowBy) {
                    return allElements.slice(0, rowBy).reduce(((acc, item) => Math.max(acc, item.offsetHeight)), 0) + offset;
                }
                const applyIndentTop = height => {
                    itemsBody.style.marginTop = `-${height}px`;
                };
                const applyScale = element => {
                    if (scaleElement) element.style.transform = `scale(${scaleElement})`;
                };
                const resetStyles = element => {
                    element.removeAttribute("style");
                };
                itemsForMoving.forEach((item => {
                    applyScale(item);
                    applyIndentTop(elementsHeight);
                }));
                const moveElements = async () => {
                    rowBy = getRowBy(itemsBody);
                    allElements = Array.from(itemsBody.children);
                    const itemsForMoving = allElements.slice(0, rowBy);
                    const itemsForPrepend = allElements.slice(-rowBy).reverse();
                    offset = parseFloat(window.getComputedStyle(itemsBody).getPropertyValue("row-gap"));
                    const elementsHeight = getElementsHeight(allElements, rowBy);
                    const elementsPrependHeight = getElementsHeight(itemsForPrepend, rowBy);
                    applyIndentTop(elementsHeight);
                    itemsBody.style.transition = `transform ${animationDuration}ms ${animTransitionTimingFunction}`;
                    itemsBody.style.transform = `translateY(${elementsHeight}px)`;
                    if (itemsForMoving && scaleElement) itemsForMoving.forEach((item => {
                        item.style.transition = `transform ${animationDuration}ms ${animTransitionTimingFunction}`;
                        item.style.transform = `scale(1)`;
                    }));
                    await new Promise((resolve => setTimeout(resolve, animationDuration)));
                    applyIndentTop(elementsPrependHeight);
                    itemsBody.style.transition = `none`;
                    itemsBody.style.transform = "translateY(0)";
                    if (itemsForMoving && scaleElement) itemsForMoving.forEach(resetStyles);
                    if (rowBy && itemsForMoving) {
                        const fragment = document.createDocumentFragment();
                        itemsForPrepend.forEach((item => {
                            applyScale(item);
                            fragment.prepend(item.cloneNode(true));
                            item.remove();
                        }));
                        itemsBody.prepend(fragment);
                    }
                    setTimeout(moveElements, duration);
                };
                setTimeout(moveElements, delayStart);
            }));
        }), 50);
    }
    function rowHorizontalAnim() {
        const container = document.querySelector("[data-row-anim]");
        let sourceList = document.querySelector("[data-row-anim-list]");
        if (container && sourceList) {
            const maxRowsArray = sourceList.dataset.maxRows?.split(",");
            let maxRows = getRowsMinMax();
            const additionItems = +sourceList.dataset.additionItems || 2;
            const scrollSpeed = +sourceList.dataset.scrollSpeed || .5;
            let items = Array.from(sourceList.children);
            const gap = parseFloat(window.getComputedStyle(sourceList).getPropertyValue("column-gap"));
            function hasEnoughItemsForOneRow(items) {
                const containerWidth = container.offsetWidth;
                let totalWidth = 0;
                for (let i = 0; i < items.length; i++) {
                    totalWidth += items[i].offsetWidth + gap;
                    if (totalWidth > containerWidth && i - 1 + additionItems <= items.length && items[i - 1 + additionItems]) return true;
                }
                return false;
            }
            function distributeElements(elements, rows) {
                const rowsCount = rows.length;
                elements.forEach(((element, index) => {
                    const rowIndex = rowsCount - 1 - index % rowsCount;
                    rows[rowIndex]?.push(element);
                }));
                return rows;
            }
            function createRows(items, maxRows, isResize) {
                const containerWidth = container.offsetWidth;
                isResize ? sourceList = Array.from(container.children) : null;
                isResize ? items = Array.from(container.querySelectorAll("li")) : null;
                let rows = [];
                let currentRow = [];
                let currentRowWidth = 0;
                for (let index = 0; index < items.length; index++) if (rows.length < maxRows) {
                    const item = items[index];
                    const itemWidth = item.offsetWidth + gap;
                    if (currentRowWidth + itemWidth > containerWidth) {
                        const remainingItems = items.slice(index);
                        if (remainingItems.length >= additionItems) {
                            currentRow.push(...remainingItems.slice(0, additionItems));
                            index = index - 1 + additionItems;
                            rows.push(currentRow);
                            currentRow = [];
                            currentRowWidth = 0;
                        } else currentRow.push(...remainingItems);
                    } else {
                        currentRow.push(item);
                        currentRowWidth += itemWidth;
                    }
                } else {
                    let leftItemsNotFitInRows = items.slice(index);
                    rows = distributeElements(leftItemsNotFitInRows, rows);
                    break;
                }
                if (currentRow.length > 0 && hasEnoughItemsForOneRow(currentRow)) rows.push(currentRow); else if (currentRow.length > 0) rows = distributeElements(currentRow, rows);
                rows.forEach(((rowItems, index) => {
                    const row = document.createElement("ul");
                    row.classList.add("city-list__items");
                    row.setAttribute("data-row-anim-list", "");
                    rowItems.forEach((item => {
                        const clone = item.cloneNode(true);
                        row.appendChild(clone);
                    }));
                    container.appendChild(row);
                    startScroll(row, index % 2 === 0);
                }));
                Array.isArray(sourceList) ? sourceList.forEach((list => list.remove())) : sourceList.remove();
                return rows;
            }
            function startScroll(row, isLeft) {
                let position = 0;
                function step() {
                    position += isLeft ? -scrollSpeed : scrollSpeed;
                    ...
                    if (isLeft && firstItemRect.right <= containerRect.left) {
                        position += firstItem.offsetWidth + gap;
                        row.appendChild(firstItem);
                        row.style.transform = `translateX(${position}px)`;
                    } else if (!isLeft && lastItemRect.left >= containerRect.right) {
                        position -= lastItem.offsetWidth + gap;
                        row.insertBefore(lastItem, firstItem);
                        row.style.transform = `translateX(${position}px)`;
                    }
            ...
            function getRowsMinMax() {
                return window.innerWidth >= 479.98 ? +maxRowsArray[0] || 4 : +maxRowsArray[1] || 4;
            }
            if (!hasEnoughItemsForOneRow(items)) {
                container.parentElement.classList.add("_no-enough");
                return;
            } else {
                createRows(items, maxRows);
                const optimizedResize = debounce((() => {
                    maxRows = getRowsMinMax();
                    createRows(items, maxRows, true);
                }), 150);
    ...
    function testimonialsSlider() {
        const sliderBlock = document.querySelector('[data-slider="testimonials-slider"]');
        if (sliderBlock) {
