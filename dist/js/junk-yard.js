(() => {
    ...
    function selectDayPopularTimes() {
        const animDuration = "0.4s";
        const animTimingFunc = "ease-in-out";
        const wrapper = document.querySelector("[data-popular-times]");
        if (wrapper) {
            const select = wrapper.querySelector("[data-select-day]");
            const days = Array.from(wrapper.querySelectorAll("[data-day]"));
            setCurrentDayAsSelected(select);
            let currentValue = select.value;
            const currentHour = (new Date).getHours();
            days.forEach((day => {
                day.style.display = "none";
                if (day.dataset.day === currentValue) day.style.display = "flex";
                highlightCurrentTime(day, currentHour);
            }));
            select.addEventListener("change", (e => {
                const currentDay = days.find((day => day.dataset.day === currentValue));
                const selectedDay = days.find((day => day.dataset.day === select.value));
                if (currentDay && selectedDay) switchDay(currentDay, selectedDay);
                currentValue = select.value;
            }));
        }
        function switchDay(oldDay, newDay) {
            const oldDayPersentElements = oldDay.querySelectorAll(".popular-times__time-percent");
            const newDayPersentElements = newDay.querySelectorAll(".popular-times__time-percent");
            const heightsOldDay = Array.from(oldDayPersentElements).map((elem => elem.style.height));
            const heightsNewDay = Array.from(newDayPersentElements).map((elem => elem.style.height));
            newDayPersentElements.forEach(((elem, index) => {
                elem.style.height = heightsOldDay[index];
            ...
            setTimeout((() => {
                newDayPersentElements.forEach(((elem, index) => {
                    elem.style.transition = `height ${animDuration} ${animTimingFunc}`;
                    elem.style.height = heightsNewDay[index];
                }));
            }), 50);
        }
        ...
        function setCurrentDayAsSelected(select) {
            ...
            select.setAttribute("data-selected", todayIndex);
            select.value = todayIndex;
            const options = select.options;
            for (let i = 0; i < options.length; i++) if (parseInt(options[i].value) === todayIndex) {
                options[i].textContent = "Today";
                break;
            }
        }
    }
    ...
    function junkYardSlider() {
        const sliderBlock = document.querySelector('[data-slider="main-content-slider"]');
        if (sliderBlock) {
            ...
            if (slider) new swiper_core_Swiper(slider, {
                ...
                on: {
                    lock: () => slider.classList.add("swiper--lock"),
